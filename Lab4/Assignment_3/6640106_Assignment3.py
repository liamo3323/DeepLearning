...
class ImageEncoder(nn.Module):
    ...
    def __init__(
        ...
        for path in self.model.parameters():
            path.requires_grad = trainable
...
def find_matches(model, image_embeddings, query, image_filenames, n=9):
    ...
    image_embeddings_n = F.normalize(image_embeddings, path=2, dim=-1)
    text_embeddings_n = F.normalize(text_embeddings, path=2, dim=-1)
...
if __name__ == "__main__":
    ...
    # setting up the model to use be in evaluation
    ...
    # setting up the Text tokenizer 
    tokenizer = DistilBertTokenizer.from_pretrained(CFG.text_tokenizer)

    # setting up the image embeddings
    for d in tqdm(os.listdir(p)):
        ds = []

        # reading the captions from each path in the directory
        ...
        # reading the images from each path in the directory
        ...
        # creating a dataloader for the images
        dataloader = torch.utils.data.DataLoader(
            images,
            batch_size=CFG.batch_size,
            num_workers=CFG.num_workers,
        ...
        # getting the matches for each group of images
        for g in tqdm(groups):
            captions, image_embeds, images_path = groups[g]
            
            n = 1

            # getting the matches for each caption
            for caption in captions:
                ...
                # matching the images to the captions
                with torch.no_grad():
                    text_features = model.text_encoder(
                        input_ids=batch["input_ids"], attention_mask=batch["attention_mask"]
                    )
                ...
                results.append((matches[0].replace(f'{urn}/', ''), caption))
